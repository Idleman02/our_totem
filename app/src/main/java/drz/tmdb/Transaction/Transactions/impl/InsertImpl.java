...
public class InsertImpl implements Insert {
    ...
    private Integer insert(int classId, List<String> columns, Tuple tuple, int l, int[] index) throws TMDBException {
        ...
        ArrayList<Integer> idlist = tJoinDeputySize(classId);
        //首先调用tJoinDeputySize方法得到是否当前类是否存在基于轨迹相似度的代理类，并得到代理类的id list
        if(!idlist.isEmpty()) {
            //如果list非空就需要进一步判断
            List<Coordinate> traj1 = TrajTrans.getTraj(tuple.tuple[2].toString());
            //调用TrajTrans.getTraj方法将当前元祖的轨迹部分转化为List<Coordinate> traj1进行后续操作
            for (Integer deputyId : idlist) {
                //对代理类idlist进行遍历
                int tempClassId = deputyId;
                //得到当前代理类的id
                int anotherClassId = getAnotherDeputy(deputyId, classId);
                //由于基于轨迹相似度的代理类需要两个源类，需要拿到另一个源类，这里使用getAnotherDeputy方法拿到
                TupleList tuples = select.getTable(classId);
                //通过select的getTable（classid）方法拿到另一个源类的所有tuple
                for(int i = 0; i < tuples.tuplenum; i++) {
                    //遍历另一个源类的所有tuple
                    Tuple tmptul = tuples.tuplelist.get(i);
                    //拿到另一个源类的当前tuple
                    List<Coordinate> traj2 = TrajTrans.getTraj(tmptul.tuple[2].toString());
                    //通过TrajTrans.getTraj方法将当前元祖的轨迹部分转化为List<Coordinate> traj2进行后续操作
                    List<Coordinate> common = longestCommonSubSequence.getCommonSubsequence(traj1,traj2,3);
                    ...
                    if (common.size()>3){
                        ...
                        String temps=TrajTrans.getString(common);
                        temp1.tuple[2]=temps;
